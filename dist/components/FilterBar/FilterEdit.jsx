...
export const FilterEdit = (props) => {
  const [selectedBoolValue, setSelectedBoolValue] = createSignal(
  ...
  const [selectedTextValue, setSelectedTextValue] = createSignal(
    props.filter.field?.type === "text" ? props.filter.value : "",
  );
  const [selectedNumberValue, setSelectedNumberValue] = createSignal(
    props.filter.field?.type === "number" ? props.filter.value : 0,
  );
  const [selectedSelectValue, setSelectedSelectValue] = createSignal(
    props.filter.field?.type === "select" ? props.filter.value : null,
  );
  const [selectedDateValues, setSelectedDateValues] = createStore(
    props.filter.field?.type === "date" ? props.filter.value : { startDate: null, endDate: null },
  );
